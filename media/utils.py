...
def enoughSpace(request, space_used):
	...
	return size_gb + space_used < u.space
...
def filterCategoriesByPermissions(request, q):
	"""
	@param q: Initial Q() object
	@return Queryset restricted by access levels
	"""
...
def getUserGroups(username):
	...
	try:
		...
		print e
	users=l.search_ext_s("dc=Mycompany,dc=com", ldap.SCOPE_SUBTREE, "(sAMAccountName="+username+")", attrlist=['memberOf'])
	if username:
		try:
...
def crowdAuth(request):
	from media.crowd import *
	from media_server.settings import CROWD_URL, CROWD_USER, CROWD_PASSWORD
	import os, sys, getpass
	
	if 'Mycompanydash_token_key' in request.COOKIES:
		token = request.COOKIES['Mycompanydash_token_key']
		x_forwarded_for = request.META.get('HTTP_X_FORWARDED_FOR')
		ip = '10.253.0.56'
		# Create the reusable Crowd object
		cs = CrowdServer(CROWD_URL, CROWD_USER, CROWD_PASSWORD)
		
		success = cs.validate_session(token, remote=ip)
		if success:	
